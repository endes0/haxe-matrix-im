...
class Presence extends Handler {
  ...
  public var list(default, set): Map<String, User_status> = new Map();

  public function set_list(list: Map<String, User_status>): Map<String, User_status> {
      var remove = [];
      var add = [];
...
public function edit_list(user: User, add: Array<String>, remove: Array<String>, ?on_response: Void -> Void) : Void {
...
public function update_list(user: User, time: Int, ?on_response: Array<{content: User_status, type: Types}> -> Void) : Void {
    this.send_request(Conection.make_request(HttpMethod.GET, server + '/_matrix/client/r0/presence/list/' + user , null), function ( status : Int, data: Array<{content: User_status, type: Types}>) : Void {
        for(e in data) {
            this.list[e.content.user_id] = e.content;
        }
        if(on_response != null) on_response(data);
    });
