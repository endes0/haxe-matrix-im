...
class Rooms extends Handler {
  ...
  public var typing_on: Map<String, Bool> = new Map();
  ...
  public function check_typing(time: Int, user: User) {
    for(room in typing_on.keys()) {
      if(typing_on[room] == true) {
        this.typing(new Room(room), user, time + 200, function(t:Bool) {});
      } else if(typing_on[room] == false) {
        this.typing(new Room(room), user, time, true, function(t:Bool) {});
        typing_on[room] == null;
      }
    }
  ...
  public inline function invite( room : Room, user : User, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/invite', {user_id: user.toString()}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }
  ...
  public inline function third_invite( room : Room, identity_server : String, address: String, medium: String = 'email', on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/invite', {id_server: identity_server, address: address, medium: medium}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }
  ...
  public inline function join( room : Room, ?third_party_signed : {sender: String, mxid: String, token: String, signatures: Dynamic}, on_response : Room -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/join', if(third_party_signed != null) {third_party_signed: third_party_signed} else null), function( status : Int, data : Dynamic ) : Void {
      on_response(new Room(data.room_id));
    });
  }

  public inline function leave( room : Room, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/leave', null), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function forget( room : Room, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/forget', null), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function kick( room : Room, user : User, reason : String, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/kick', {user_id: user.toString(), reason: reason}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function ban( room : Room, user : User, reason : String, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/ban', {user_id: user.toString(), reason: reason}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function unban( room : Room, user : User, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('POST', this.server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/ban', {user_id: user.toString()}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }
  ...
  public inline function add_alias( alias : String, room : Room, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('PUT', server + '/_matrix/client/r0/directory/room/%23' + alias.urlEncode() + '%3A' + room.get_server(), {room_id : room.toString()}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function remove_alias( alias : String, server : String, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('DELETE', this.server + '/_matrix/client/r0/directory/room/%23' + alias.urlEncode() + '%3A' + server, null), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function get_room_from_alias( alias : String, server : String, on_response : Room -> Array<String> -> Void ) : Void {
    this.send_request(Conection.make_request('GET', this.server + '/_matrix/client/r0/directory/room/%23' + alias.urlEncode() + '%3A' + server, null), function( status : Int, data : Dynamic ) : Void {
  ...
  public inline function get_state_event_content( room : Room, type : Types, ?key : String, on_response : Dynamic -> Void ) : Void {
    this.get_room_events_content('/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/state/' + type + '/' + if(key != null) key.urlEncode() else '', on_response);
  }

  public inline function get_state_events( room : Room, on_response : Array<Event<Dynamic>> -> Void ) : Void {
    this.get_room_events('/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/state', on_response);
  }
  ...
  public inline function get_events( room : Room, ?from_token: String, ?timeout: Int, on_response : String -> String -> Array<Event<Dynamic>> -> Void ) : Void {
    this.send_request(Conection.make_request('GET', server + '/_matrix/client/r0/events?room_id=' + room.toString().urlEncode() + (if(timeout != null) '&timeout=' + timeout else '') + (if(from_token != null) '&from=' + from_token.toString().urlEncode() else ''), null),
  ...
  public inline function get_members( room : Room, on_response : Array<Event<m.room.Member>> -> Void ) : Void {
    this.send_request(Conection.make_request('GET', server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/members', null), function( status : Int, data : Dynamic ) : Void {
  ...
  public inline function get_joined_members( room : Room, on_response : Map<String,{display_name: String, avatar_url: String}> -> Void ) : Void {
    this.send_request(Conection.make_request('GET', server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/joined_members', null), function( status : Int, data : Dynamic ) : Void {
  ...
  public inline function get_messages( room : Room, from : String, ?to : String, dir : Directions, limit : Int = 10, ?filter : Dynamic, on_response : String -> String -> Array<Event<Dynamic>> -> Void ) : Void {
    this.send_request(Conection.make_request('GET', server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/messages?from=' + from.urlEncode() + (if(to != null) to.urlEncode() else '') + '&dir=' + dir + '&limit=' + limit + (if(filter != null) filter else '') , null), function( status : Int, data : Dynamic ) : Void {
      on_response(data.start, data.end, data.chunk);
    });
  }

  public inline function send_state_event( room : Room, type : Types, ?key : String, content : Dynamic, on_response : String -> Void ) : Void {
    this.send_request(Conection.make_request('PUT', server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/state/' + type + '/' + if(key != null) key else '', content), function( status : Int, data : Dynamic ) : Void {
  ...
  public inline function send_event( room : Room, type : Types, ?txnid : String, content : Dynamic, on_response : String -> Void ) : Void {
    ...
    this.send_request(Conection.make_request('PUT', server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/send/' + type + '/' + txnid.urlEncode(), content), function( status : Int, data : Dynamic ) : Void {
      on_response(data.event_id);
    });
  }
  ...
  public inline function redact_event( room : Room, event_id : String, ?txnid : String, reason : String, on_response : String -> Void ) : Void {
    ...
    this.send_request(Conection.make_request('PUT', server + '/_matrix/client/r0/rooms/' + room.toString().urlEncode() + '/redact/' + event_id.urlEncode() + '/' + txnid.urlEncode(), {reason: reason}), function( status : Int, data : Dynamic ) : Void {
      on_response(data.event_id);
    });
  }
  ...
  public inline function get_tags( room : Room, user: User, on_response : Array<m.Tag> -> Void ) : Void {
    this.send_request(Conection.make_request('GET', server + '/_matrix/client/r0/user/' + user.toString().urlEncode() + '/rooms/' + room.toString().urlEncode() + '/tags', null), function( status : Int, data : {tags: Array<Dynamic>} ) : Void {
  ...
  public inline function add_tag( room : Room, user: User, tag: String, order: Int, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('PUT', server + '/_matrix/client/r0/user/' + user.toString().urlEncode() + '/rooms/' + room.toString().urlEncode() + '/tags/' + tag.urlEncode(), {order: order}), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }

  public inline function remove_tag( room : Room, user: User, tag: String, on_response : Void -> Void ) : Void {
    this.send_request(Conection.make_request('DELETE', server + '/_matrix/client/r0/user/' + user.toString().urlEncode() + '/rooms/' + room.toString().urlEncode() + '/tags/' + tag.urlEncode(), null), function( status : Int, data : Dynamic ) : Void {
      on_response();
    });
  }
